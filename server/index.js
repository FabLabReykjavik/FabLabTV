...
app.post("/api/audio/request-enable", async (_req, res) => {
  const state = await loadState();
  await saveState({
    audio: {
      ...(state.audio || {}),
  ...
  io.emit("command", { type: "audioRequestEnable" });
  await broadcastStatus();
  res.json(await buildStatus());
});

app.post("/api/audio/approved", async (_req, res) => {
  await saveState({
    audio: {
      requested: false,
      enabled: true,
  ...
  io.emit("command", { type: "audioSetMuted", muted: false });
  await broadcastStatus();
  res.json(await buildStatus());
});

app.post("/api/audio/disable", async (_req, res) => {
  const state = await loadState();
  await saveState({
    audio: {
      ...(state.audio || {}),
  ...
  io.emit("command", { type: "audioSetMuted", muted: true });
  await broadcastStatus();
  res.json(await buildStatus());
});

app.post("/api/audio/enable", async (_req, res) => {
  const state = await loadState();
